...
if [[ "${host_os}" != "Linux" || "${host_arch}" != "aarch64" ]]; then
  echo "This script builds a Linux aarch64 wheel and must run on a Linux aarch64 host."
...
if ! library_present; then
  ...
  if [ ! -d "${build_dir}" ]; then
    meson setup "${build_dir}" --libdir=lib/aarch64-linux-gnu -Dprefix="${install_dir}" "${repo_root}"
  else
    meson setup --reconfigure "${build_dir}" --libdir=lib/aarch64-linux-gnu -Dprefix="${install_dir}" "${repo_root}"
  fi
